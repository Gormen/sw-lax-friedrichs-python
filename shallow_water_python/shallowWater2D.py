...
def shallowWater(n,XMAX,TMAX):
	...
	while tsum<TMAX:
		...
		hu[1:-1,1:-1] -= lambd*( .5 * ( ( Rhuf[:,:-1]-Rhuf[:,1:] ) + ( Rhug[:-1,:]-Rhug[1:,:] ) ) )# update cell average (tip: depends on Rh and lambda)
		hv[1:-1,1:-1] -= lambd*( .5 * ( ( Rhvf[:,:-1]-Rhvf[:,1:] ) + ( Rhvg[:-1,:]- Rhvg[1:,:] ) ) )# update cell average (tip: depends on Rh and lambda)
...
def plot3d(fig,mX,mY,h,n):
	...
	#ax.plot_surface(mX, mY, h, rstride=n/25, cstride=n/25, cmap=cm.YlGnBu_r)
        ax.plot_surface(mX, mY, h, rstride=n/25, cstride=n/25, cmap=cm.Blues)
	#ax.plot_surface(mX, mY, h, rstride=n/25, cstride=n/25, cmap=cm.GnBu)
...
def initialize(x,y,XMAX):
	...
	# Gaussian 2D distribution (two peaks) (m.exp( -((x - c)**2 + (y - c)**2) / (2*(r)**2) ))
	...
	for i in range(0, len(h) / 2):
        ...
        for i in range(len(h) / 2, len(h)):
            for j in range(0, len(h[i])):
                h[i][j] = h[i][j] = m.exp( -((x[j] - c)**2 + (y[i] - c)**2) / (2*r**2) ) * 1.0
...
def fluxesG(h,hu,hv):
	...
	return gh,ghu,ghv,maxeig1
	#return h,hu,hv,.0

def LxFfluxF(q, fqp, fqm, lambd):
        LxFF = .5 * ( (fqm + fqp) - .5 / lambd * (q[1:-1,:-1] - q[1:-1,1:]) )
        #LxFF = .5 * ( (fqm + fqp) - 1. / lambd * (q[1:-1,:-1] - q[1:-1,1:]) )
       
	return LxFF
	#return np.zeros(LxFF.shape)

def LxFfluxG(q, gqo, gqn, lambd):
        LxFG = .5 * ( (gqo + gqn) - .5 / lambd * (q[:-1,1:-1] - q[1:,1:-1]) )
        #LxFG = .5 * ( (gqo + gqn) - 1. / lambd * (q[:-1,1:-1] - q[1:,1:-1]) )
        
        return LxFG
	#return np.zeros(LxFG.shape)
